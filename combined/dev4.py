...
# -------------------------------
# Wallbox polling thread
# -------------------------------
def poll_wallbox():
    global shared_data
    while True:
        try:
            status = w.getChargerStatus(CHARGER_ID)
            with data_lock:
                shared_data["wallbox"] = {
                    "charger_status": Statuses(status["status"]).name,
                    "current_amp": status["config_data"]["max_charging_current"],
                    "voltage": status["default_voltage"]
                }
        except Exception as e:
            print(f"[Wallbox] Error: {e}")
        time.sleep(5)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=poll_wallbox, daemon=True).start()
    threading.Thread(target=poll_can, daemon=True).start()
...
# 4. Replace all your print with logger calls:
...
START_OF_LOWEST_PRICE_ELEC = time(23,59) # 11:59 pm
...
def safe_get_status():
    ...
    try:
        ...
        if e.response.status_code == 401:
            logger.info("Wallbox code 401, Token expired, re-authenticating...")
            w.authenticate()  # This should refresh your token
            authentication_time = now 
            return w.getChargerStatus(chargerId)  # Retry after auth
        else:
            logger.error("⚠️  wallbox error code not known =  {e.response.status_code}")
            raise


w.authenticate()  #initial authentication
while True:   
    now = datetime.now(timezone.utc)
    local_time = now.astimezone(local_tz)  
    ...
    if local_time >= next_time:
        ...
        logger.info(f" ")
        #logger.info(f"time: {local_time.strftime('%H:%M')}")
        logger.info(f"status {car_charger_status} car_charging rate {current_car_charging_rate}")
        logger.info(f"home_SOC={soc:2.0f}%, home Charge Rate= {measured_home_battery_charge_rate:2.1f}")     
        ...
        #logger.info(f"Needed house battery power: {needed_home_battery_power:.2f} kWh")
        ...
        if hours_remaining <= 0 or current_car_charging_rate <= 0:
            #logger.info
            # ("Time remaining is less than 0, setting to 1 hour")            
            optimum_home_battery_charge_rate = 0 
        else:
            optimum_home_battery_charge_rate = needed_home_battery_power / hours_remaining
        logger.info(f"Optimum Home battery charge rate: {optimum_home_battery_charge_rate:2.2f} kW")

        if START_OF_DAY_TIME < local_time.time() < END_OF_DAY_TIME:
            if first_loop_after_charging_time == True:
                logger.info(f"First iteration during daytime range")
                first_loop_after_charging_time = False                
            #logger.info("Time is between 8:30 AM and 3:00 PM")
            if soc>SOC_VALUE_CONSIDERED_FULL:  #fix for non liniarity in SOC data                 
                 soc=100
                 if measured_home_battery_charge_rate < 0:   #discharging
                      #logger.info(f"home battery full, home battery discharging")
                      if car_charger_status == "CHARGING":
                            #logger.info(f"car is charging at rate {current_car_charging_rate:2.2f} A")
                            ...
                            if new_car_charging_rate >= CAR_CHARGE_RATE_MIN:                                 
                                 ...
                                 logger.info(f"new car charging rate {new_car_charging_rate:2.2f} A")
                            else:
                                 logger.info(f"car charging rate too low, stop charging car")
                                 w.pauseChargingSession(chargerId)     
                      else:
                            logger.info(f"continue not charging car")
                 else:                                    #charging or dumping to grid      
                      #logger.info(f"home battery full, home battery charging or dumping to the grid")
                      if car_charger_status == "CHARGING":
                            #logger.info(f"car charging at rate {current_car_charging_rate:2.2f} A")
                            ...
                            if new_car_charging_rate < CAR_CHARGE_RATE_MAX:
                                 ...
                                 logger.info(f"car charging rate maxed no change")
                      else:
                            #logger.info(f"car not charging, lets try charging car")
                            ...
                            if new_car_charging_rate <= CAR_CHARGE_RATE_MAX:
                                 w.setMaxChargingCurrent(chargerId, new_car_charging_rate)
                                 logger.info(f"new car charging rate {new_car_charging_rate:2.2f} A")
                                 logger.info(f"Starting car charging with current status =  {car_charger_status}")
                                 if car_charger_dictionary['status_id'] in [209, 164, 162]:
                                   w.resumeChargingSession(chargerId)  
                                 else:
                                   logger.info(f"Status code is not good for resume charging, status={car_charger_status}")  
                            else:
                                 logger.info(f"insufficent charge rate to start car charging")
                                   
            else: #home battery not full
                 if car_charger_status == "CHARGING":
                      logger.info(f"home battery not full, car is charging @ rate {current_car_charging_rate:2.2f} A")
                      if measured_home_battery_charge_rate < optimum_home_battery_charge_rate:
                           logger.info(f"car charging too fast @ rate {current_car_charging_rate:2.2f} A")
                           ...
                           if new_car_charging_rate > CAR_CHARGE_RATE_MIN:
                                ...
                                logger.info(f"not enough power to charge car at min rate")
                                w.pauseChargingSession(chargerId)
                      else:
                           logger.info(f"home battery charging too fast even with car charging at {current_car_charging_rate:2.2f} A")
                           new_car_charging_rate = current_car_charging_rate - (optimum_home_battery_charge_rate-measured_home_battery_charge_rate)*1000/240
                           if new_car_charging_rate < CAR_CHARGE_RATE_MAX:
                                w.setMaxChargingCurrent(chargerId, new_car_charging_rate)
                                logger.info(f"new car charging rate {new_car_charging_rate:2.2f} A")  
                           else:
                                logger.info(f"charger current maxed out no change")
                 else:
                      logger.info(f"home battery not full, car not charging")  
                      if measured_home_battery_charge_rate >= optimum_home_battery_charge_rate+CAR_CHARGE_RATE_MIN*240/1000:
                           logger.info(f"home battery charging too fast -lets try to charge car")
                           ...
                           if CAR_CHARGE_RATE_MAX > new_car_charging_rate >= CAR_CHARGE_RATE_MIN:
                                ...
                                w.setMaxChargingCurrent(chargerId, new_car_charging_rate)
                                logger.info(f"Starting car charging with current status =  {car_charger_status}")
                                logger.info(f"car charger status id =  {car_charger_dictionary['status_id']}")
                                if car_charger_dictionary['status_id'] in [209, 164, 162, 182]: 
                                    w.resumeChargingSession(chargerId)
                                else:
                                    logger.info(f"Status code is not good for resume charging, status={car_charger_status}")       
                           else:    
                           ...
                           logger.info(f"home battery charging slowly, continue no car charging")
        else:
           if END_OF_DAY_TIME < local_time.time() < START_OF_LOWEST_PRICE_ELEC:       #first_loop_after_charging_time == False:
              logger.info(f"outside of daytime range = pause car charging")
              first_loop_after_charging_time = True 
              if car_charger_status == "CHARGING":
                     logger.info(f"probably pluged car in after daytime charging period")
                     logger.info(f"pausing charging until morning")
                     w.pauseChargingSession(chargerId)
              else:
                     logger.info(f"car not charging, no change")
              
           else:   
              logger.info("Night time, no changes in car charging")                                
    ...
    if message is not None and (message.arbitration_id == 0x803F01 or message.arbitration_id == 0x23F01):        
        if message.arbitration_id == 0x803F01 and state == "initial" and message.data[0] == 0xC6 :# and message.data[2] == 0xD0 :  
            ...
            number=0
                    ...
                    #logger.info
                    # (f"B03 = {B03:7d}, B47 = {B47:7d}", end=" ")
                    #logger.info
                    # (f"ID: {(message.arbitration_id)}, Data: {[(b) for b in message.data]}")   
