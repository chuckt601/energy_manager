...
CAR_CHARGE_RATE_MIN = 1.44 # kw eqivalent to 6 amps
CAR_CHARGE_RATE_MAX = 9.6 # kw equivelent to 40 amps
...
while 1:
    ...
    if local_time >= next_time:
        ...
        print(f"home_battery_SOC= {soc:7.2f}%, measured home Battery Charge Rate= {measured_home_battery_charge_rate:7.2f}")     
        ...
        if START_OF_DAY_TIME < local_time.time() < END_OF_DAY_TIME:
            ...
            print(f"Home battery charge rate: {home_battery_charge_rate:.2f} kW")
            
            if measured_home_battery_charge_rate+current_car_charging_rate> optimum_home_battery_charge_rate+CAR_CHARGE_RATE_MIN:
                 current_car_charging_rate = home_battery_charge_rate - measured_home_battery_charge_rate 
                 print(f"Car charging rate: {current_:.2f} kW")
                 car_charging_state = "true"
                 current_car_charging_rate = max(current_,CAR_CHARGE_RATE_MIN)
            else:
