...
class WallboxCharger:
    ...
    def safe_get_status(self):
         ...
         try:
             ...
             if e.response.status_code == 401:
                 self.logger.info("Wallbox code 401, Token expired, re-authenticating...")
                 self.wallbox.authenticate()  # This should refresh your token
                 self.authentication_time = self.now 

                 return self.wallbox.getChargerStatus(chargerId)  # Retry after auth
             else:
    ...
    def _poll_loop(self):
        while True:
            try:
                ...
                self.latest_data["car_charger_status code"] =car_charger_status_code
    ...
    def set_new_charging_rate(self, new_charging_rate):
        try:
            self.wallbox.setChargingCurrent(config.WALLBOX_CHARGER_ID, new_charging_rate)
            self.logger.info(f"New charging rate set to {new_charging_rate}A")
        except requests.exceptions.HTTPError as e:
            self.logger.error(f"Error setting new charging rate: {e}")      
    def pause_charging(self):
        try:
            self.wallbox.w.pauseChargingSession(config.WALLBOX_CHARGER_ID)
    ...
    def resume_charging(self):
        try:
            if self.latest_data["car_charger_status_code"] in [209, 164, 162, 182]:                                       
                self.wallbox.w.resumeChargingSession(config.WALLBOX_CHARGER_ID)
                self.logger.info("Charging resumed")                
                return   
            self.wallbox.w.resumeChargingSession(chargerId)            
        except requests.exceptions.HTTPError as e:
